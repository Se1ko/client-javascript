...
class RPClient {
   ...
   */
    ...
    startLaunch(launchDataRQ) {
        ...
        if (launchDataRQ.id) {
            this.duplMap[tempId] = this.map[tempId] = this.getNewItemObj(resolve => resolve(launchDataRQ));
            this.duplMap[tempId].readId = this.map[tempId].realId = launchDataRQ.id;
            ...
            this.duplMap[tempId] = this.map[tempId] = this.getNewItemObj((resolve, reject) => {
                const url = 'launch';
    ...
    startTestItem(testItemDataRQ, launchTempId, parentTempId) {
        ...
        if (parentTempId) {
            ...
            let parentObj = this.map[parentTempId];
            if (!parentObj) {
                parentObj = this.duplMap[parentTempId]
                parentLost = true;
        ...
        this.duplMap[tempId] = this.map[tempId] = this.getNewItemObj((resolve, reject) => {
            parentPromise.then(() => {
                ...
                let url = 'item/';
                if (parentTempId) {
                    const realParentId = (parentLost == false) ? this.map[parentTempId].realId : this.duplMap[parentTempId].readId
                    url += `${realParentId}`;
                }
        ...
        this.duplMap[parentMapId].children.push(tempId);

        if (!testItemDataRQ.retry) {
            this.nonRetriedItemMap.set(itemKey, this.map[tempId].promiseStart || this.duplMap[tempId].promiseStart);
        }
    ...
    finishTestItem(itemTempId, finishTestItemRQ) {
        const itemObj = this.map[itemTempId] || this.duplMap[itemTempId];
        //console.log(itemObj.children)
        //console.log(`CLIENT FINISH: ${itemTempId} ${finishTestItemRQ}`);
        if (!itemObj) {
        ...
        Promise.all(itemObj.children.map(itemId => this.map[itemId].promiseFinish || this.duplMap[itemId].promiseFinish))
            .then(() => {
